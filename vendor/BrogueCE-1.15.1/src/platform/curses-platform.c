...
/* brogue-bot: cached "are we driven by the bot" check */
static int bb_botMode(void) {
    static int mode = -1;
    if (mode == -1) {
        mode = getenv("BROGUE_READY_FILE") ? 1 : 0;
    }
    return mode;
}

static boolean curses_pauseForMilliseconds(short milliseconds, PauseBehavior behavior) {
    /* brogue-bot: animation delays are pointless wall time, and the
       intermediate frames are never parsed (the driver only reads the
       screen after the ready sentinel, which follows its own refresh) —
       skip both so each action renders exactly one frame */
    if (bb_botMode()) {
        return Term.hasKey();
    }
    Term.refresh();
...
/* brogue-bot: emit one byte whenever the game starts waiting for player
   input, so an external driver can synchronize without screen-polling.
   Enabled only when BROGUE_READY_FILE is set in the environment. */
#include <fcntl.h>
#include <unistd.h>
#include <sys/select.h>
static int bb_readyFd = -2;
static void bb_emitReady(void) {
    if (bb_readyFd == -2) {
        const char *bb_path = getenv("BROGUE_READY_FILE");
        bb_readyFd = bb_path ? open(bb_path, O_WRONLY|O_CREAT|O_APPEND, 0644) : -1;
    }
    if (bb_readyFd >= 0) {
        ssize_t bb_rc = write(bb_readyFd, "R", 1);
        (void)bb_rc;
    }
}
static void curses_nextKeyOrMouseEvent(rogueEvent *returnEvent, boolean textInput, boolean colorsDance) {
    ...
    Term.refresh();
    /* only signal readiness when nothing we were sent remains unconsumed:
       one sentinel byte == "all input processed, game idle, your move".
       Emitted AFTER the refresh so the final frame's bytes reach the
       terminal before the sentinel: the driver needs no settle margin. */
    if (!Term.hasKey()) {
        bb_emitReady();
    }
    ...
    for (;;) {
        ...
        if (bb_botMode()) {
            /* brogue-bot: block until stdin has bytes instead of polling
               every 34ms; the timeout is only a safety net in case ncurses
               buffered a partial sequence select() can't see */
            fd_set bb_fds;
            struct timeval bb_tv = { 0, 50000 };
            FD_ZERO(&bb_fds);
            FD_SET(STDIN_FILENO, &bb_fds);
            select(STDIN_FILENO + 1, &bb_fds, NULL, NULL, &bb_tv);
        } else {
